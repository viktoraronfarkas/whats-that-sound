...
import { setToCollection } from "./store/viewSlice";
...
const Tracker = () => {
  ...
  const getFlightPath = (callsign) => {
    fetch(
      ...
      .then((data) => {
        console.log(data);
        if (data.data.length && typeof data.data !== "String") {
          console.log(data.data);
          setDeparture(`${data.data[0].departure.airport.slice(0, 12)}...`);
          setArrival(`${data.data[0].arrival.airport.slice(0, 12)}...`);
        } else {
          setDeparture("N/A");
          setArrival("N/A");
        }
  ...
  const saveToCollection = () => {
    ...
    localStorage.setItem("collection", JSON.stringify(savedItems));
    setIsSaved(true);
  };
      ...
      return (
        <div>
          <a
            ...
            rel="noreferrer"
          >
            <button className="primary large bottom-margin">
              <i>travel</i>
  ...
  function SaveBtn() {
    if (!isSaved) {
      ...
      return (
        <button className="secondary large" disabled={true}>
          <i>bookmark_added</i>
    ...
    if (unknown) {
      ...
      return (
        <div>
          ...
          <img
            src={aircraftData.aircraft.url_photo ?? flyingKitty}
            height="100"
          ></img>
        </div>
      );
  ...
  return (
    <div className="App">
      <div className="full-width right-align" hidden={view === "collection"}>
        <button
          className="toggle-page bottom-margin right-align"
